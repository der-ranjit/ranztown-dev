// the ide does not know about the type, since it infers the types from the root tsconfig.json
// @ts-ignore
        ...
        if (f) {
            if (upload.fileName) {
                mv(f.path, upload.fileName, (err) => {
                    if (err) {
                        finish(err.message, null as any);
                        return;
                    }

                    finish(null as any, upload.fileName);
                });
            } else {
                fs.readFile(f.path, (err, data) => {
                    ...
                    fs.unlink(f.path, (err) => {
                        finish(null as any, `data:${f.type};base64,${data.toString('base64')}`);
                    });
                });
            }
        }
...
app.use(koaBody({
    ...
    // server upload folder for nui app; this way clients can load resources that are not pre-defined in the manifest
   .use(mount("/storage/upload", serve(staticDirPath)))
